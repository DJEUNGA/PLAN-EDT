  ...
  return (req, res, next) => {
    if (!rolesPermis.includes(req.utilisateur.role)) {
      return res.status(403).json({
        erreur: `Accès refusé. Rôle requis : ${rolesPermis.join(' ou ')}.`,
      });
    }
    next();
  };
...
app.post('/api/auth/inscription', async (req, res) => {
  ...
  if (!rolesValides.includes(role)) {
    return res.status(400).json({ erreur: 'Rôle invalide. Choisir : admin, professeur ou etudiant.' });
  }
...
app.get('/api/matieres', authentifier, (req, res) => {
  try {
    res.json(db.prepare('SELECT * FROM matieres').all());
  } catch (err) {
...
app.post('/api/matieres', authentifier, autoriser('admin'), (req, res) => {
  ...
  if (![4, 8].includes(Number(duree_cours))) {
    return res.status(400).json({ erreur: 'duree_cours doit être 4 ou 8 heures.' });
  }
...
app.put('/api/matieres/:id', authentifier, autoriser('admin'), (req, res) => {
  ...
  try {
    db.prepare(
      'UPDATE matieres SET nom = ?, heures_semaine = ?, duree_cours = ?, couleur = ? WHERE id = ?'
...
app.get('/api/professeurs', authentifier, (req, res) => {
  try {
    res.json(db.prepare('SELECT * FROM professeurs').all());
  } catch (err) {
    res.status(500).json({ erreur: err.message });
  }
});
...
app.post('/api/professeurs', authentifier, autoriser('admin'), (req, res) => {
  ...
  try {
    const joursJSON = jours_disponibles ? JSON.stringify(jours_disponibles) : null;
    const result    = db.prepare(
      'INSERT INTO professeurs (nom, matiere_id, salle, jours_disponibles) VALUES (?, ?, ?, ?)'
    ).run(nom, matiere_id || null, salle || null, joursJSON);
...
app.put('/api/professeurs/:id', authentifier, autoriser('admin'), (req, res) => {
  ...
  try {
    const joursJSON = jours_disponibles ? JSON.stringify(jours_disponibles) : null;
    db.prepare(
      'UPDATE professeurs SET nom = ?, matiere_id = ?, salle = ?, jours_disponibles = ? WHERE id = ?'
    ).run(nom, matiere_id, salle, joursJSON, req.params.id);
...
app.delete('/api/professeurs/:id', authentifier, autoriser('admin'), (req, res) => {
  try {
    db.prepare('DELETE FROM professeurs WHERE id = ?').run(req.params.id);
...
// GET version enrichie avec noms (via algorithm.js)
app.get('/api/emplois/detail', authentifier, (req, res) => {
  ...
  try {
    ...
    res.json(getEmploiDuTemps(Number(classe_id), Number(semaine) || 1));
  } catch (err) {
    res.status(500).json({ erreur: err.message });
  }
});

// POST génération automatique
app.post('/api/emplois/generer', authentifier, autoriser('admin'), (req, res) => {
  const { semaine } = req.body;
  if (!semaine) return res.status(400).json({ erreur: 'semaine est requise (ex: 1, 2, 3...).' });

  try {
    const { genererEmploiDuTemps } = require('./algorithm');
    const resultat = genererEmploiDuTemps(Number(semaine));
    res.json({ message: `Emploi du temps généré : ${resultat.length} créneaux.`, total: resultat.length });
  } catch (err) {
    res.status(500).json({ erreur: 'Erreur lors de la génération.', details: err.message });
  }
});
...
app.delete('/api/emplois/:id', authentifier, autoriser('admin'), (req, res) => {
  try {
    db.prepare('DELETE FROM emplois_du_temps WHERE id = ?').run(req.params.id);
...
// Absence préventive — prof déclare à l'avance
app.post('/api/absences/preventive', authentifier, autoriser('admin', 'professeur'), (req, res) => {
  ...
  if (req.utilisateur.role === 'professeur' &&
      req.utilisateur.professeur_id !== professeur_id) {
...
// Absence constatée — admin ou étudiant signale
app.post('/api/absences/constatee', authentifier, autoriser('admin', 'etudiant'), (req, res) => {
  ...
  try {
    ...
    const resultat = signalerAbsenceConstatee(
      professeur_id, date_absence, req.utilisateur.id, heure_debut, heure_fin
    );
...
// GET rattrapages enrichis (via algorithm.js)
...
// PUT marquer effectué ou annulé
...
app.put('/api/notifications/:id/lue', authentifier, (req, res) => {
  try {
    ...
    ).run(req.params.id, req.utilisateur.id);
...
app.get('/api/profil', authentifier, (req, res) => {
  try {
    res.json(db.prepare(
      'SELECT id, nom, email, role, classe_id, professeur_id, actif, created_at FROM utilisateurs WHERE id = ?'
    ).get(req.utilisateur.id));
  } catch (err) {
    res.status(500).json({ erreur: err.message });
  }
...
app.get('/api/utilisateurs', authentifier, autoriser('admin'), (req, res) => {
  try {
    res.json(db.prepare(
      'SELECT id, nom, email, role, classe_id, professeur_id, actif, created_at FROM utilisateurs'
    ).all());
  } catch (err) {
    res.status(500).json({ erreur: err.message });
  }
